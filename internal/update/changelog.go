...
func ChangelogURL() string {
	if v := strings.TrimSpace(os.Getenv("SD_CHANGELOG_URL")); v != "" {
		return v
	}
	return "https://raw.githubusercontent.com/Skila1/SoundDock/main/CHANGELOG.md"
}

func RemoteVersionURL() string {
	if v := strings.TrimSpace(os.Getenv("SD_VERSION_URL")); v != "" {
		return v
	}
	return "https://raw.githubusercontent.com/Skila1/SoundDock/main/VERSION"
}
...
func httpGet(ctx context.Context, cli *http.Client, rawURL string) (string, error) {
	...
	req.Header.Set("User-Agent", "SoundDock-Updater")
	res, err := cli.Do(req)
	if err != nil {
		return "", err
	}
