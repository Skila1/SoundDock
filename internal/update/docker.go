...
func skipUpdate(c containerSummary) bool {
	...
	if svc == "postgres" || strings.Contains(img, "postgres") {
		return true
	}
	if strings.Contains(name, "update-swap") || strings.Contains(name, "-next") {
		return true
	}
	if svc == "sounddock" || svc == "discord-worker" {
		return false
	}
	return !strings.Contains(img, "sounddock")
}
...
func spawnSwapper(ctx context.Context, jobs []swapJob) error {
	...
	_ = dockerDoDiscard(ctx, http.MethodDelete, "/v1.41/containers/sounddock-update-swap?force=1", nil)
	cfg := map[string]any{
		"Image": self.Image,
		"Cmd":   []string{"update-swap"},
		"Env":   append(append([]string{}, self.Env...), "SD_UPDATE_SWAP="+string(payload)),
	...
	cr, err := dockerDo(ctx, http.MethodPost, "/v1.41/containers/create?name=sounddock-update-swap", bytes.NewReader(raw))
	if err != nil {
		return err
	}
	defer cr.Body.Close()
...
func RunSwap() error {
	raw := strings.TrimSpace(os.Getenv("SD_UPDATE_SWAP"))
	if raw == "" {
		return fmt.Errorf("SD_UPDATE_SWAP is empty")
	}
	var jobs []swapJob
