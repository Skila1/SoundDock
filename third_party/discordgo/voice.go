...
// Speaking sends Opcode 5. Discord voice gateway v4+ requires an integer
// bitmask and the connection SSRC. A boolean payload without ssrc is ignored
// and other clients never mix the bot's UDP packets.
...
func (v *VoiceConnection) Speaking(b bool) (err error) {
	...
	flags := 0
	if b {
		flags = 1 // Microphone — normal voice transmission
	}
	data := voiceSpeakingOp{5, voiceSpeakingData{flags, 0, v.op2.SSRC}}
	v.wsMu.Lock()
...
func (v *VoiceConnection) WaitForDAVEReady(ctx context.Context) error {
	// Do not treat dave==nil as ready. After 2026-03-01 Discord requires DAVE;
	// returning early here starts PCM before EncryptFrame exists, so listeners hear silence.
	...
	for {
		...
		if v.dave != nil && v.dave.CanEncrypt() {
			return nil
		}
		v.Cond.Wait()
	}
...
func (v *VoiceConnection) onEvent(ctx context.Context, binary bool, message []byte) {
	...
	if binary {
		...
		case 4: // udp encryption secret key
			...
			// discord.py + Discord docs: Opcode 5 with ssrc must land before UDP audio.
			_ = v.Speaking(false)
			return
...
func (v *VoiceConnection) opusSender(ctx context.Context, rate, size int) {
	...
	for i := uint32(0); true; i++ {
		...
		v.Cond.L.Lock()
		dave := v.dave
		daveRequired := v.op4.DAVEProtocolVersion > 0
		...
		for daveRequired && !daveActive {
			if sampleLog {
				v.log(LogWarning, "opus sender holding frame until DAVE CanEncrypt idx=%d", i)
			}
			select {
			case <-ctx.Done():
				return
